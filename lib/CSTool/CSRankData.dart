...
class CSUserData {
  ...
  factory CSUserData.fromJson(Map<String, dynamic> json) {
    return CSUserData(
...
class CSUserDataManager {
  static const String _lastUpdateKey = 'lastUpdate';
  static const String _dataKey = 'CSUserData';
  static final Random _random = Random();

  /// 生成随机用户数据
  static List<CSUserData> generateCSUserData() {
    List<CSUserData> list = [];
  ...
  /// 保存数据到本地
  static Future<void> saveCSUserData(List<CSUserData> data) async {
  ...
  /// 加载本地数据
  static Future<List<CSUserData>> loadCSUserData() async {
    final prefs = await SharedPreferences.getInstance();
    ...
    return jsonList.map((e) {
      ...
      return CSUserData.fromJson(map);
    }).toList();
  }

  /// 获取数据，如果当天没更新则刷新
  static Future<List<CSUserData>> getCSUserData() async {
    final prefs = await SharedPreferences.getInstance();
    ...
    if (needUpdate) {
      List<CSUserData> newData = generateCSUserData();
      await saveCSUserData(newData);
      return newData;
    } else {
      return await loadCSUserData();
    }
  }
}