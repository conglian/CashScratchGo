...
class CSGradientNumberRoller extends StatefulWidget {
  ...
  @override
  _CSGradientNumberRollerState createState() => _CSGradientNumberRollerState();
}

class _CSGradientNumberRollerState extends State<CSGradientNumberRoller>
  ...
  @override
  void didUpdateWidget(CSGradientNumberRoller oldWidget) {
  ...
  String _formatNumber(num number) {
    ...
    if (number is int && widget.decimalPlaces == 0) {
      return '${widget.showDolas ? '${0.dolasType()}' : ''}${number.toString()}';
    }
    // 否则按小数处理，保留指定小数位数
    return '${widget.showDolas ? '${0.dolasType()}' : ''}${number.toStringAsFixed(widget.decimalPlaces)}';
  }
