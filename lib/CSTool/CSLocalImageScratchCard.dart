...
class CSLocalImageScratchCard extends StatefulWidget {
  final Widget child;
  ...
  @override
  _CSLocalImageScratchCardState createState() => _CSLocalImageScratchCardState();
}

class _CSLocalImageScratchCardState extends State<CSLocalImageScratchCard> with TickerProviderStateMixin {
  List<Offset> _points = [];
  ...
  void initState() {
    ...
    CSScratchUpdateNotificationService.stream.listen((value) async {
      CSScratchTapNotificationService.sendToDomandNumberNotification(0);
      if (mounted && value == 0) {
        _resetScratchCard();
      } else if (value == 1) {
...
class CSScratchUpdateNotificationService {
...
class CSScratchTapNotificationService {
  static final StreamController<int> _streamController = StreamController<int>.broadcast();
